...
class MigrateReport
{
  ...
  /** @var string File that contains URL list, either yaml or json. */
  private $urlsSource;
  ...
  public function __construct(SymfonyStyle $io, $dstDomain, $srcDomain=null, array $options=[])
  {
    ...
    $filenameSuffix = ($options['filename_suffix'] ?? null);
   ...
   * @param null $appendix
   */
  private function setFilenameRoot($appendix=null) {
    $fn = rtrim($this->dstDomain, '/');
    $fn = preg_replace('/[^a-z0-9]+/','-', strtolower($fn));
    $fn = "merlin-report-{$this->reportType}";
    if (!empty($appendix)) {
      $appendix = preg_replace('/[^a-z0-9]+/','-', strtolower($appendix));
      $fn .= "-{$appendix}";
    }
  ...
  public function setUrlsSource($source) {
    ...
    if ($this->adoptSourceName) {
      $source = $this->urlsSource;
      $parts = pathinfo($source);
      $name = $parts['filename'];

      // Set filename suffix.
      $this->setFilenameRoot($name);
